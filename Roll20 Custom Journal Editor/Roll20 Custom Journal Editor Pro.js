...
// @grant        none
// @updateURL    https://raw.githubusercontent.com/OHgolgyeo/script/refs/heads/main/Roll20%20Custom%20Journal%20Editor/Roll20%20Custom%20Journal%20Editor%20Pro.js
...
(function() {
    ...
    const buildEmbeddedStyleBlock = () => {
        ...
        const fontImports = urls.map(u => `@import url('${u}');`).join('');
        return '<style>' + fontImports + R20_HANDOUT_AVATAR_HIDE_CSS + '</style>';
    };
    ...
    const openFontSettingsPopover = (anchorBtn) => {
        ...
        pop.querySelector('.r20-font-save-btn').addEventListener('click', (e) => {
            ...
            if (workingFonts.length === 0 || uniqueSizes.length === 0) {
                alert('폰트와 크기가 각각 1개 이상 있어야 저장할 수 있습니다.');
                return;
            }
